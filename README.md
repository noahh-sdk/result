...
A result class for C++ implemented mainly for Noahh.
...
int main() {
    ...
    if (NOAHH_UNWRAP_IF_OK(p3, integerDivision(3, 2))) {
        assert(p3 == 1);
    }

    int p4 = 0;
    if (NOAHH_UNWRAP_INTO_IF_OK(p4, integerDivision(3, 2))) {
        assert(p4 == 1);
    }
    ...
    if (NOAHH_UNWRAP_IF_ERR(e1, integerDivision(3, 0))) {
        assert(e1 == "Division by zero");
    }

    std::string e2;
    if (NOAHH_UNWRAP_INTO_IF_ERR(e2, integerDivision(3, 0))) {
    ...
    if (NOAHH_UNWRAP_EITHER(p5, err, integerDivision(3, 2))) {
        assert(p5 == 1);
    } else {
        assert(false);
    }

    if (NOAHH_UNWRAP_EITHER(p6, err, integerDivision(3, 0))) {
        assert(false);
    } else {
    ...
    int p7 = 0;
    std::string e3;
    if (NOAHH_UNWRAP_INTO_EITHER(p7, e3, integerDivision(3, 2))) {
        assert(p7 == 1);
    } else {
        assert(false);
    }
...
int main() {
    ...
    // otherwise sets the value into the variable
    // Requires both ok and err values to be default constructible
    NOAHH_UNWRAP_OR_ELSE(p8, err, integerDivision(3, 2)) {
        return -1;
    }
    assert(p8 == 1);

    NOAHH_UNWRAP_OR_ELSE(p9, err, integerDivision(3, 0)) {
        p9 = -1;
    }
    assert(p9 == -1);

    int p10 = 0;
    NOAHH_UNWRAP_INTO_OR_ELSE(p10, err, integerDivision(3, 2)) {
        return -1;
    }
    assert(p10 == 1);
}
...
int main() {
    ...
    // otherwise returns the result of itself
    if (NOAHH_UNWRAP_IF_OK(v5, integerDivision(3, 2).map([](int v){
        return v * 2;
    }))) {
        assert(v5 == 2);
    }
    ...
    // otherwise returns the result of itself
    if (NOAHH_UNWRAP_IF_ERR(e1, integerDivision(3, 0).mapErr([](auto const& e){
        return e + "!";
    }))) {
        assert(e1 == "Division by zero!");
    }
