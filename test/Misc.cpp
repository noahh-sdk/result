...
// Test the special NOAHH_UNWRAP macro on void unwrap
...
TEST_CASE("Misc") {
    ...
    SECTION("Unwrap If Macros") {
        SECTION("Ok") {
            auto res = divideConstexpr(32, 2);
            if (NOAHH_UNWRAP_IF_OK(value, res)) {
                REQUIRE(value == 16);
            }
            else {
            ...
            if (NOAHH_UNWRAP_IF_ERR(value, res)) {
        ...
        SECTION("Some") {
            auto res = divideConstexpr(32, 2);
            if (NOAHH_UNWRAP_IF_SOME(value, res.ok())) {
                REQUIRE(value == 16);
            }
            else {
                FAIL("Expected the block to be executed");
            ...
            if (NOAHH_UNWRAP_IF_SOME(value, res.err())) {
                FAIL("Expected the block to not be executed");
            }
            else {
                REQUIRE(true);
    ...
    SECTION("Unwrap Into Else") {
        NOAHH_UNWRAP_OR_ELSE(value, err, divideConstexpr(32, 2)) {
            FAIL("Expected the block to not be executed");
        }
        REQUIRE(value == 16);

        NOAHH_UNWRAP_OR_ELSE(value2, err, divideConstexpr(32, 0)) {
            REQUIRE(err == -1);
        }
    }

    SECTION("Unwrap Either") {
        if (NOAHH_UNWRAP_EITHER(value, err, divideConstexpr(32, 2))) {
            REQUIRE(value == 16);
        }
        else {
            FAIL("Expected the block to be executed");
        ...
        if (NOAHH_UNWRAP_EITHER(value, err, divideConstexpr(32, 0))) {
            FAIL("Expected the block to not be executed");
        }
        else {
            REQUIRE(err == -1);
